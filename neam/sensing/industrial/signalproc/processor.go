...
func (p *Processor) processPoint(point TelemetryPoint) *TelemetryPoint {
	...
	numericValue, isNumeric := toFloat64(point.Value)

	p.mu.Lock()
	state, exists := p.pointState[point.NodeID]
	if !exists {
		state = &PointState{
			ValueHistory: make([]float64, 0, p.cfg.SpikeWindowSize),
		}
		p.pointState[point.NodeID] = state
	}
	state.mu.Lock()
	p.mu.Unlock()
	...
	if isNumeric && !p.checkRange(numericValue) {
		state.mu.Unlock()
	...
	if !exists {
		if isNumeric {
			state.LastValue = numericValue
			state.LastTimestamp = point.Timestamp
			state.LastReportedValue = numericValue
	...
	// Deadband filter (only for numeric values)
	if isNumeric && p.applyDeadband(numericValue, state) {
		state.mu.Unlock()
		p.incrementStat("filtered")
		return nil
	}

	// Outlier detection (only for numeric values)
	if isNumeric && p.detectOutlier(numericValue, state) {
		state.OutlierCount++
		state.mu.Unlock()
		p.incrementStat("outliers_detected")
		p.logger.Debug("Outlier detected",
			zap.String("node_id", point.NodeID),
			zap.Float64("value", numericValue))
	} else {
	...
	if isNumeric {
		spikeDetected := p.detectSpike(numericValue, state)
		if spikeDetected {
			state.SpikeCount++
		...
		if !p.checkRateOfChange(numericValue, state) {
			state.mu.Unlock()
			p.incrementStat("filtered")
			return nil
		}
		...
		// Add to history
		state.ValueHistory = append(state.ValueHistory, numericValue)
...
func (p *Processor) applyDeadband(point float64, state *PointState) bool {
	if state.LastReportedValue == 0 {
		return false
	}

	// Calculate percentage change
	change := (point - state.LastReportedValue) / state.LastReportedValue * 100
	change = abs(change)
...
func (p *Processor) detectSpike(point float64, state *PointState) bool {
...
// detectOutlier identifies statistical outliers using z-score
func (p *Processor) detectOutlier(point float64, state *PointState) bool {
	if len(state.ValueHistory) < 5 {
		return false
	}
	...
	mean := sum / float64(len(state.ValueHistory))
	...
	stdDev := sqrt(variance / float64(len(state.ValueHistory)))
	...
	// Calculate z-score
	zScore := abs(point - mean) / stdDev

	return zScore >= p.cfg.OutlierThreshold
}

// checkRateOfChange validates rate of change limit
func (p *Processor) checkRateOfChange(point float64, state *PointState) bool {
	...
	valueDelta := point - state.LastValue
...
func (p *Processor) GetPointState(nodeID string) *PointState {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.pointState[nodeID]
}

// ClearPointState removes state for a specific point
func (p *Processor) ClearPointState(nodeID string) {
	p.mu.Lock()
	defer p.mu.Unlock()
	delete(p.pointState, nodeID)
}
